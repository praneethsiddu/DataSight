# -*- coding: utf-8 -*-
"""
Created on Sat Jan 22 20:10:50 2022
    ...
    if 'main_data.csv' not in os.listdir('data'):
        ...
        if st.button("Generate Models"):
            ...
            # Save one of the models 
                if dt_r2 > lr_r2:
                # save decision tree 
                    joblib.dump(dt_model, '{pred_type}.joblib')
                else: 
                    joblib.dump(lr_model, '{pred_type}.joblib')

            # Make a dataframe of results 
                results = pd.DataFrame(model_r2, columns=['Models', 'R2 Score']).sort_values(by='R2 Score', ascending=False)
                st.dataframe(results)
                st.success("Now Go Do Predictions with your top Model")
            ...
            # Decision Tree model 
                ...
                model_acc.append(['Decision Tree Classification', dtc_acc])

            # Save one of the models 
                if dtc_acc > lc_acc:
                # save decision tree 
                    joblib.dump(dtc_model,'{pred_type}.joblib')
                else: 
                    joblib.dump(lc_model, '{pred_type}.joblib')
